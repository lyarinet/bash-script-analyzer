...
const App: React.FC = () => {
  ...
  const handleApplyFix = useCallback((originalCode: string, refactoredCode: string) => {
    setScriptContent(prev => prev.replace(originalCode, refactoredCode));
  }, []);

  const handleApplyAllFixes = useCallback((fixes: RefactorResponse[]) => {
    let currentScript = scriptContent;
    fixes.forEach(fix => {
        // Simple sequential replacement. This might fail if replacements overlap,
        // but it's a good starting point for this feature.
