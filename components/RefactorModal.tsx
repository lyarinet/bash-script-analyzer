...
const RefactorModal: React.FC<RefactorModalProps> = ({ isOpen, isLoading, error, data, suggestion, onClose, onApplyFix }) => {
  useEffect(() => {
    if (!isOpen) return;
  ...
  const handleApply = () => {
    if (data) {
        onApplyFix(data.originalCode, data.refactoredCode);
        onClose();
    }
  };
        ...
        <main className="p-6 overflow-y-auto w-full min-h-0">
          ...
          {data && (
            <div className="animate-fade-in">
              <p className='text-gray-400 mb-4 italic'>Suggestion: "{suggestion}"</p>
